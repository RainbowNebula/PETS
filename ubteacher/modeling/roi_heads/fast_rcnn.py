...
def fast_rcnn_inference_single_image(
    ...
    result.object_logits = temp_scores[filter_inds[:, 0]]
    ...
    def __init__(
        ...
        if len(proposals):
            ...
            if proposals[0].has("gt_classes"):
                ...
                # self.gt_scores = cat([p.objectness_logits for p in proposals], dim=0)
                # If "gt_boxes" does not exist, the proposals must be all negative and
                # should not be included in regression loss computation.
                # Here we just use proposal_boxes as an arbitrary placeholder because its
                # value won't be used in self.box_reg_loss().
...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        #soft_target = torch.ones(input.shape)
        #CE = self.soft_cross_entropy(input, target)
        p = torch.exp(-CE)
