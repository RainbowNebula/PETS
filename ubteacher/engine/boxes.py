...
def update_box(boxes1, boxes2):
    #  When the shapes do not match,
    #  the shape of the returned output tensor follows the broadcasting rules
    lt = torch.max(boxes1[:, :2], boxes2[:, :2])  # left-top [N,M,2]
    rb = torch.min(boxes1[:, 2:], boxes2[:, 2:])  # right-bottom [N,M,2]

    new_boxes = torch.cat([lt, rb],dim=1)

    # 框大小取两个box的均值
    w_ratio = (boxes1[:,2] - boxes1[:,0] + boxes2[:,2] - boxes2[:,0]) / (2 * (new_boxes[:,2] - new_boxes[:,0]))
    h_ratio = (boxes1[:,3] - boxes1[:,1] + boxes2[:,3] - boxes2[:,1]) / (2 * (new_boxes[:,3] - new_boxes[:,1]))

    new_boxes[:,[0,2]] = new_boxes[:,[0,2]] * w_ratio[:,None]
    new_boxes[:,[1,3]] = new_boxes[:,[1,3]] * h_ratio[:,None]
