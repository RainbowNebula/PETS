...
class AspectRatioGroupedDatasetTwoCrop(AspectRatioGroupedDataset):
    ...
    def __init__(self, dataset, batch_size):
        ...
        self._buckets_key_1 = [[] for _ in range(2)]
    ...
    def __iter__(self):
        for d in self.dataset:
            ...
            buckets_key_1 = self._buckets_key_1[bucket_id]
            buckets_key_1.append(d[2])
            if len(bucket) == self.batch_size:
                yield (bucket[:], buckets_key[:], buckets_key_1[:])
                del bucket[:]
                del buckets_key[:]
                del buckets_key_1[:]
...
class AspectRatioGroupedSemiSupDatasetTwoCrop(AspectRatioGroupedDataset):
    ...
    def __init__(self, dataset, batch_size):
        ...
        self._unlabel_buckets_key1 = [[] for _ in range(2)]
        # Hard-coded two aspect ratio groups: w > h and w < h.
        # Can add support for more aspect ratio groups, but doesn't seem useful
    ...
    def __iter__(self):
        ...
        for d_label, d_unlabel in zip(self.label_dataset, self.unlabel_dataset):
            ...
            if len(unlabel_bucket) != self.batch_size_unlabel:
                ...
                unlabel_buckets_key1 = self._unlabel_buckets_key1[unlabel_bucket_id]
                unlabel_buckets_key1.append(d_unlabel[2])
            ...
            if (
                ...
                yield (
                    ...
                    unlabel_bucket[:],
                    unlabel_buckets_key[:],
                    unlabel_buckets_key1[:],
                )
                del label_bucket[:]
                del label_buckets_key[:]
                del unlabel_bucket[:]
                del unlabel_buckets_key[:]
                del unlabel_buckets_key1[:]
