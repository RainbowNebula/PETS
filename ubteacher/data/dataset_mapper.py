...
from ubteacher.data.detection_utils import build_strong_augmentation, build_weak_augmentation
...
class DatasetMapperTwoCropSeparate(DatasetMapper):
    ...
    def __init__(self, cfg, is_train=True):
        ...
        # ================ 控制不同View的开关 ======================= #
        self.weak_augmentation = build_weak_augmentation(cfg, is_train)
    ...
    def __call__(self, dataset_dict):
        ...
        dataset_dict_weak = copy.deepcopy(dataset_dict)
        dataset_dict_weak["image"] = torch.as_tensor(
            np.ascontiguousarray(self.weak_augmentation(image_pil)).transpose(2, 0, 1)
        )
        assert dataset_dict_weak["image"].size(1) == dataset_dict_weak["image"].size(1)
        return (dataset_dict, dataset_dict_key, dataset_dict_weak)
