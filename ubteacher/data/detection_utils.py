...
def build_strong_augmentation(cfg, is_train):
    ...
    if is_train:
        ...
        # augmentation.append(EqualizeHist())  #直方图均衡化
        augmentation.append(
        ...
        #                                            p=0.2))
        # PT: https://github.com/hikvision-research/ProbabilisticTeacher/blob/main/pt/data/detection_utils.py
        ...
        #augmentation.append(RandomMask(2, mask_ratio=0.25))
        # 和DenseTeacher一致
        logger.info("Augmentations used in training: " + str(augmentation))
    return transforms.Compose(augmentation)


def build_weak_augmentation(cfg, is_train):
    """
    Create a list of :class:`Augmentation` from config.
    Now it includes resizing and flipping.
    ...
    if is_train:
        ...
        #     transforms.RandomApply([transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)], p=0.8),
        # )
        # augmentation.append(transforms.Compose([
        #     transforms.ToTensor(),
        #     transforms.ToPILImage(),
        # ]))
        # augmentation.append(transforms.RandomApply([Solarize(threshold=0.5)],
